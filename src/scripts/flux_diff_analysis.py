    ...
    for file in files:
        ...
        while ix < len(fluxes):
            ...
            for reaction, flux in fluxes[ix].items():
                ...
                flux_dict[rxn][class_label].setdefault(qualifier, [])
                flux_dict[rxn][class_label][qualifier].append(flux)
        ...
        for reaction, flux_vals in flux_dict.items():
            healthy_flux = (sum(flux_vals[healthy]['min'])/len(flux_vals[healthy]['min']),
                            sum(flux_vals[healthy]['max'])/len(flux_vals[healthy]['max']))

            diseased_flux = (sum(flux_vals[diseased]['min'])/len(flux_vals[diseased]['min']),
                             sum(flux_vals[diseased]['max'])/len(flux_vals[diseased]['max']))
        ...
        #subsystem_distances = [(max(distances), subsys) for subsys, distances in subsystem_dist_dict.items()]
        subsystem_distances = [(sum(sorted(distances, reverse=True)[:min(3, len(distances))]) / min(3, len(distances)), subsys) for subsys, distances in
                               subsystem_dist_dict.items()]
